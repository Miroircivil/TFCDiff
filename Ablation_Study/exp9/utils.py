    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(train_loader) as it:
            for batch_no, (clean_batch, noisy_batch) in enumerate(it, start=1):
                ...
                loss = model(clean_batch, noisy_batch, epoch_no=epoch_no)
                loss.backward()
        ...
        if valid_loader is not None and (epoch_no + 1) % valid_epoch_interval == 0:
            ...
            #if epoch_no % 50 == 0:  # <-- check if update alpha
            #    best_valid_loss = 1e10  # <-- reset to 1e10
            #    print(f"\nReset best_valid_loss at epoch {epoch_no} (alpha changes).")
...
def evaluate(model, test_loader, shots, device, foldername=""):
    ...
    with tqdm(test_loader) as it:
        for batch_no, (clean_batch, noisy_batch) in enumerate(it, start=1):
            ...
            noisy_batch, _= dwt(noisy_batch_0)
            noisy_batch = noisy_batch / eta
            if shots > 1:
                output = 0
            ...
            temp = [torch.zeros(*output.shape[:-1], output.shape[-1]*2, device=output.device, dtype=output.dtype), torch.zeros_like(output)]
            output = idwt((output, temp))
...
class metrics:
    ...
    def SNR_improvement(y_in, y_out, y_clean):
        return metrics.SNR(y_clean, y_out) - metrics.SNR(y_clean, y_in)
