...
dwt = DWT1DForward(J=2, wave='db4', mode='periodization').cuda() 
idwt = DWT1DInverse(wave='db4', mode='periodization').cuda()
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        _, _, length = x.shape
        x = self.norm(x)
        temp = [torch.zeros(*x.shape[:-1], x.shape[-1]*2, device=x.device, dtype=x.dtype), torch.zeros_like(x)]
        x = idwt((x, temp))
        x = self.up(x)
        x, _ = dwt(x)
        return self.conv(x)
...
class TFE(nn.Module):
    ...
    def forward(self, x, time_emb):
        ...
        h = self.block1(x)
        h = self.noise_func(h, time_emb)
        temp = [torch.zeros(*h.shape[:-1], h.shape[-1]*2, device=h.device, dtype=h.dtype), torch.zeros_like(h),]
        h = idwt((h, temp))
        h = self.block2(h)
        h, _ = dwt(h)
        return h + self.res_conv(x)
...
class TFF(nn.Module):
    ...
    def forward(self, input):
        ...
        # qkv = self.qkv(norm).view(batch, n_head, head_dim * 3, seq_len)
        qkv = self.qkv(norm)
        query, key, value = qkv.chunk(3, dim=1)  # bncl

        temp = [torch.zeros(*query.shape[:-1], query.shape[-1]*2, device=query.device, dtype=query.dtype), torch.zeros_like(query),]
        q = idwt((query, temp)).view(batch, n_head, head_dim, -1)
        k = idwt((key, temp)).view(batch, n_head, head_dim, -1)
        v = idwt((value, temp)).view(batch, n_head, head_dim, -1)

        attn = torch.einsum(
            "bncl, bncL -> bnlL", q, k
        ).contiguous() / math.sqrt(channel)
        attn = attn.view(batch, n_head, int(seq_len*4), -1)
        attn = torch.softmax(attn, -1)
        attn = attn.view(batch, n_head, int(seq_len*4), int(seq_len*4))

        out = torch.einsum("bnlL, bncL -> bncl", attn, v).contiguous()

        out, _= dwt(out.view(batch, channel, -1))
        out = self.out(out)
        ...
        for ind in reversed(range(num_mults)):
            ...
            for _ in range(0, res_blocks+1):
                ...
                ups.append(ResnetBlocWithAttn(
                    pre_channel+feat_channel, channel_mult, noise_level_emb_dim=noise_level_channel, norm_groups=norm_groups,
    ...
    def forward(self, x, time):
        ...
        for layer in self.ups:
            if isinstance(layer, SELayer):
                ...
                x = layer(torch.cat((x, feats_concat), dim=1), t)
            else:
                x = layer(x)
