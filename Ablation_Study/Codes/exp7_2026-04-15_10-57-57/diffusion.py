...
class DDPM(nn.Module):
    def __init__(self, base_model, config, device, conditional=True):
        ...
        self.loss_func = self.hybrid_loss
    ...
    def hybrid_loss(self, noise, x_recon, alpha=1, eps=1e-4):
        err = noise - x_recon
        temp_loss = self.charbonnier_loss(err, eps).sum()

        # dct
        err_freq = dct.dct(err, norm='ortho')

        # Loss in frequency domain
        freq_loss = self.charbonnier_loss(err_freq, eps).sum()
