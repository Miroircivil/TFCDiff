...
def calculate_correlation(x, y, axis=1):
    ...
    cov = tf.reduce_mean(x_dev * y_dev, axis=axis)
    
    x_std = tf.math.reduce_std(x, axis=axis)
    y_std = tf.math.reduce_std(y, axis=axis)
    
    correlation = cov / (x_std * y_std + 1e-8)
...
def combined_huber_freq_loss(y_true,y_pred):
    ...
    correlations = calculate_correlation(
        power_spectrum_orig,
        power_spectrum_denoised,
        axis=1
    )
    
    similarity = tf.reduce_mean(correlations)
...
def train_dl(Dataset, experiment, signal_size=sigLen):
    ...
    # checkpoint
    model_filepath = model_label + '_.best.weights.h5'
    ...
    # model.save(model_label + '_.best.weights.h5')
...
def test_dl(Dataset, experiment, signal_size=3600):
