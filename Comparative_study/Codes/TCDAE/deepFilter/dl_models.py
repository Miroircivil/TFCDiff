...
def spatial_attention(inputs):
    attention = keras.layers.Dense(1, activation='tanh')(inputs)
    attention = keras.layers.Flatten()(attention)
    attention = keras.layers.Activation('softmax')(attention)
    attention = keras.layers.Reshape((-1, 1))(attention)
    return attention
def attention_module(inputs, filters):
    x = keras.layers.Conv1D(filters, kernel_size=1, activation='relu')(inputs)
    x = keras.layers.Conv1D(filters, kernel_size=3, padding='same', activation='relu')(x)
    ...
    scaled_inputs = keras.layers.Multiply()([inputs, attention])
