...
from utils.metrics import MAD, SSD, PRD, COS_SIM, SNR_improvement
...
def swapaxes_list(input_list):
    num_exps = len(input_list)
    num_segs = max(len(seg) for seg in input_list) if input_list else 0

    swapped_list = [[] for _ in range(num_segs)]

    for exp_index in range(num_exps):
        for seg_index in range(len(input_list[exp_index])):
            swapped_list[seg_index].append(input_list[exp_index][seg_index])
...
if __name__ == "__main__":
    ...
    for nv in noise_versions:
        ...
        X_test = torch.load('./X_test_simEMG.pt')[:,:3584,:]
        y_test = torch.load('./y_test_simEMG.pt')[:,:3584,:]

        X_test = X_test.numpy()/200
        y_test = y_test.numpy()/200
        ...
        Dataset = [None, None, X_test, y_test]
    ...
    train_time_list = train_time_list_nv1
    test_time_list = test_time_list_nv1
...
# Load Results Attention_Skip_DAE
    with open('test_results_' + dl_experiments[0] + '_nv1.pkl', 'rb') as input:
        test_Attention_Skip_DAE_nv1 = pickle.load(input)
    # with open('test_results_' + dl_experiments[0] + '_nv2.pkl', 'rb') as input:
    #     test_DRNN_nv2 = pickle.load(input)

    test_Attention_Skip_DAE = [test_Attention_Skip_DAE_nv1[0],
                 test_Attention_Skip_DAE_nv1[1],
                 test_Attention_Skip_DAE_nv1[2]]

    # Load Results ECA_Skip_DAE
    with open('test_results_' + dl_experiments[1] + '_nv1.pkl', 'rb') as input:
        test_ECA_Skip_DAE_nv1 = pickle.load(input)
    ...
    test_ECA_Skip_DAE = [test_ECA_Skip_DAE_nv1[0],
                    test_ECA_Skip_DAE_nv1[1],
                    test_ECA_Skip_DAE_nv1[2]]
    ...
    # #                 np.concatenate((test_FCN_DAE_nv1[1], test_FCN_DAE_nv2[1])),
    # #                 np.concatenate((test_FCN_DAE_nv1[2], test_FCN_DAE_nv2[2]))]
    # test_FCN_DAE = [test_FCN_DAE_nv1[0],
    #                 test_FCN_DAE_nv1[1],
    #                 test_FCN_DAE_nv1[2]]
    ...
    # test_Vanilla_L = [test_Vanilla_L_nv1[0],
    #                 test_Vanilla_L_nv1[1],
    #                 test_Vanilla_L_nv1[2]]
    ...
    # test_Vanilla_NL = [test_Vanilla_NL_nv1[0],
    #                   test_Vanilla_NL_nv1[1],
    #                   test_Vanilla_NL_nv1[2]]
    ...
    # test_Multibranch_LANL = [test_Multibranch_LANL_nv1[0],
    #                           test_Multibranch_LANL_nv1[1],
    #                           test_Multibranch_LANL_nv1[2]]
    ...
    # test_Multibranch_LANLD = [test_Multibranch_LANLD_nv1[0],
    #                           test_Multibranch_LANLD_nv1[1],
    #                           test_Multibranch_LANLD_nv1[2]]
    ...
    [X_test, y_test, y_pred] = test_Attention_Skip_DAE

    SSD_values_DL_Attention_Skip_DAE = SSD(y_test, y_pred)

    MAD_values_DL_Attention_Skip_DAE = MAD(y_test, y_pred)

    PRD_values_DL_Attention_Skip_DAE = PRD(y_test, y_pred)

    COS_SIM_values_DL_Attention_Skip_DAE = COS_SIM(y_test, y_pred)

    ImSNR_values_DL_Attention_Skip_DAE = SNR_improvement(X_test, y_pred, y_test)


    # Exp FCN-DAE

    [X_test, y_test, y_pred] = test_ECA_Skip_DAE

    SSD_values_DL_ECA_Skip_DAE = SSD(y_test, y_pred)

    MAD_values_DL_ECA_Skip_DAE = MAD(y_test, y_pred)

    PRD_values_DL_ECA_Skip_DAE = PRD(y_test, y_pred)

    COS_SIM_values_DL_ECA_Skip_DAE = COS_SIM(y_test, y_pred)

    ImSNR_values_DL_ECA_Skip_DAE = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_DL_FCN_DAE = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_DL_exp_1 = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_DL_exp_2 = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_DL_exp_3 = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_DL_exp_4 = SNR_improvement(X_test, y_pred, y_test)
    ...
    # ImSNR_values_FIR = SNR_improvement(X_test, y_filter, y_test)
    ...
    # ImSNR_values_IIR = SNR_improvement(X_test, y_filter, y_test)


    ####### Results Visualization #######

    SSD_all = [
               SSD_values_DL_Attention_Skip_DAE,
               SSD_values_DL_ECA_Skip_DAE,
            #    SSD_values_DL_Vanilla_DAE,
               ]

    MAD_all = [
               MAD_values_DL_Attention_Skip_DAE,
               MAD_values_DL_ECA_Skip_DAE,
            #    MAD_values_DL_Vanilla_DAE,
               ]

    PRD_all = [
               PRD_values_DL_Attention_Skip_DAE,
               PRD_values_DL_ECA_Skip_DAE,
            #    PRD_values_DL_Vanilla_DAE,
               ]

    COS_SIM_all = [
                   COS_SIM_values_DL_Attention_Skip_DAE,
                   COS_SIM_values_DL_ECA_Skip_DAE,
                #    COS_SIM_values_DL_Vanilla_DAE,
                   ]
    
    ImSNR_all = [
                 ImSNR_values_DL_Attention_Skip_DAE,
                 ImSNR_values_DL_ECA_Skip_DAE,
                #  ImSNR_values_DL_Vanilla_DAE,
                 ]


    Exp_names = dl_experiments
    
    metrics = ['SSD', 'MAD', 'PRD', 'COS_SIM', 'ImSNR']
    metric_values = [SSD_all, MAD_all, PRD_all, COS_SIM_all, ImSNR_all]
    ...
    for idx_exp in range(len(Exp_names)):
        ...
        for idx_seg in range(len(segm) - 1):
            ...
            for idx in range(len(rnd_test)):
                ...
                # ImSNR
                oua = ImSNR_all[idx_exp][idx]
                if rnd_test[idx] > segm[idx_seg] and rnd_test[idx] < segm[idx_seg + 1]:
                    ImSNR_seg[idx_seg].append(oua)
        ...
        for idx in range(len(rnd_test)):
            ...
            oua = ImSNR_all[idx_exp][idx]
            if rnd_test[idx] > segm[-2]:
                ImSNR_seg[-1].append(oua)

        ImSNR_seg_all.append(ImSNR_seg)  # [exp][seg][item]
    ...
    # # debug
    # for i, row in enumerate(SSD_seg_all):
    #     print(f"Row {i} has segment lengths:")
    #     for j, seg in enumerate(row):
    #         print(f"  Column {j}: {len(seg)} points")
            
    # lengths = set()
    # for row in SSD_seg_all:
    #     for seg in row:
    #         lengths.add(len(seg))
        
    # if len(lengths) > 1:
    #     print(f"various lengths detected: {lengths}")
        
    # SSD Table
    SSD_seg_all = swapaxes_list(SSD_seg_all)
    ...
    # MAD Table
    MAD_seg_all = swapaxes_list(MAD_seg_all)
    ...
    # PRD Table
    PRD_seg_all = swapaxes_list(PRD_seg_all)
    ...
    # COS SIM Table
    COS_SIM_seg_all = swapaxes_list(COS_SIM_seg_all)
    ...
    # ImSNR Table
    ImSNR_seg_all = swapaxes_list(ImSNR_seg_all)

    print('\n')
    print('Printing Table for different noise values on the ImSNR metric')
    vs.generate_table(seg_table_column_name, ImSNR_seg_all, Exp_names)
