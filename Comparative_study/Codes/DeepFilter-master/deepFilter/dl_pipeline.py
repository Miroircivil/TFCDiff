...
def train_dl(Dataset, experiment, signal_size=3584):
    ...
    # 修改条件判断中的损失函数引用
    if experiment == 'DRNN':
        criterion = keras.losses.MeanSquaredError()  # 使用类版本
    ...
    model.compile(
        ...
        metrics=[
            keras.metrics.MeanSquaredError(),  # 类版本
            keras.metrics.MeanAbsoluteError(),  # 类版本
            ssd_loss,
            mad_loss
        ]
    )
...
def test_dl(Dataset, experiment, signal_size=3584):
